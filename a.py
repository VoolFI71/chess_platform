...
def plot_results(t_values, y_values, x_values):
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    
    axes[0,0].plot(t_values, y_values, 'b-', linewidth=2)
    axes[0,0].set_xlabel('Время')
    axes[0,0].set_ylabel('Выходная величина y')
    axes[0,0].set_title('Изменение выходной величины во времени')
    axes[0,0].grid(True)
    
    axes[0,1].plot(t_values, x_values[:,0], 'r-', label='x1')
    axes[0,1].plot(t_values, x_values[:,1], 'g-', label='x2')
    axes[0,1].plot(t_values, x_values[:,2], 'b-', label='x3')
    axes[0,1].set_xlabel('Время')
    axes[0,1].set_ylabel('Фазовые координаты')
    axes[0,1].set_title('Фазовые координаты системы')
    axes[0,1].legend()
    axes[0,1].grid(True)
    
    axes[1,0].plot(x_values[:,0], x_values[:,1], 'purple', linewidth=2)
    axes[1,0].set_xlabel('x1')
    axes[1,0].set_ylabel('x2')
    axes[1,0].set_title('Фазовый портрет (x1-x2)')
    axes[1,0].grid(True)
    
    axes[1,1].plot(x_values[:,1], x_values[:,2], 'orange', linewidth=2)
    axes[1,1].set_xlabel('x2')
    axes[1,1].set_ylabel('x3')
    axes[1,1].set_title('Фазовый портрет (x2-x3)')
    axes[1,1].grid(True)
    
    plt.tight_layout()
    plt.savefig('system_plots.png', dpi=300)
    plt.show()
...
def main():
    ...
    print("\nПостроение графиков...")
    plot_results(t_values, y_values, x_values)

def compare_B_values():
    a3, a2, a1, a0 = 5.5, 4.5, 7.5, 1.6
    b2, b1, b0 = 6, 1.4, 16
    A, Tmod, h = 2.5, 25, 10**-2
    ...
    for B, color in zip(B_values, colors):
        ...
        initial_conditions = [0, 0, 0]
        t_values, x_values, y_values = runge_kutta_4th_order(system, t_span, initial_conditions, h)
        plt.plot(t_values, y_values, color=color, linewidth=2, label=f'B = {B}')
    
    plt.xlabel('Время')
    plt.ylabel('Выходная величина y')
    plt.title('Сравнение систем с B = ±0.15')
    plt.legend()
    plt.grid(True)
    plt.savefig('comparison_B.png', dpi=300)
    plt.show()

if __name__ == "__main__":
    main()
    print("\nСравнение с B = -0.15 и B = 0.15...")
    compare_B_values()
